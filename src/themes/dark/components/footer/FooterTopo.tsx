...
import { useReducedMotion } from "@dark/lib/useReducedMotion";
...
/* ==================================================================
   ...
   2. Token driven. The shader mixes between two colours read from
      the site's own --bg and --ink tokens rather than hardcoding
      white onto black, so it follows theme.css.
...
/* The tokens are authored as hex. Reading them at runtime rather than
   ...
   with theme.css the way HeroCanvas's hardcoded palette could. */
...
export default function FooterTopo({ className }: { className?: string }) {
    ...
    useEffect(() => {
        ...
        /* Read once. This copy only renders in the dark theme, where the
           tokens never change after load. The fallbacks are theme.css's
           --bg and --ink, for a browser that hands back an empty value. */
        {
            const cs = getComputedStyle(document.documentElement);
            const bg = readHex(cs.getPropertyValue("--bg"), [0.004, 0.004, 0.004]);
            const ink = readHex(cs.getPropertyValue("--ink"), [0.933, 0.945, 0.937]);

            gl.uniform3f(u.bg, bg[0], bg[1], bg[2]);
            gl.uniform3f(u.line, ink[0], ink[1], ink[2]);
            gl.uniform1f(u.grid, 0.13);
            gl.uniform1f(u.topo, 0.42);
        }
        ...
        /* A resize while the footer is off screen still has to land, or
           scrolling back would show a stretched frame. */
        ...
        const repaintTick = window.setInterval(repaintIfNeeded, 500);

        return () => {
            stop();
            window.clearInterval(repaintTick);
            io.disconnect();
            ro.disconnect();
            gl.deleteBuffer(buf);
            gl.deleteProgram(prog);
