...
/* Which field a visitor is hiring for. Lives in src/shared because /api/verify
   validates against it — see the note there. */
...
export const contactForm = {
    ...
    /* Where everything this form sends is delivered. Defined in src/shared so
     * the API routes read the same value — see the note there. The footer
     * reads `footer.email` below rather than this field, so the two can
     * diverge if a separate public address is ever wanted. */
    email: contactEmail,
