...
export default function FooterTopo({ className }: { className?: string }) {
    ...
    useEffect(() => {
        ...
        /* Read once. This copy only renders in the light theme, where the
           ...
           --bg and --ink, for a browser that hands back an empty value. */
        {
            ...
            const bg = readHex(cs.getPropertyValue("--bg"), [0.937, 0.945, 0.933]);
            const ink = readHex(cs.getPropertyValue("--ink"), [0.071, 0.086, 0.082]);
            ...
            /* Dark ink on a light ground carries much further than white ink
               on black, so this needs less of it than the dark theme. */
            gl.uniform1f(u.grid, 0.1);
            gl.uniform1f(u.topo, 0.28);
        }
