...
function texWall() {
  ...
  x.fillStyle = THEME.stone.wallBase; x.fillRect(0, 0, W, H);
...
function texFloor() {
  ...
  x.fillStyle = THEME.stone.floorBase; x.fillRect(0, 0, W, H);
  const N = 4, S = W / N;
...
function texLacquer() {
  ...
  /* the coat */
  x.globalAlpha = .80; x.fillStyle = THEME.torii.coat; x.fillRect(0, 0, W, H);
  x.globalAlpha = 1;
...
function texSky() {
  ...
  THEME.sky.stops.forEach(s => g.addColorStop(s[0], s[1]));
...
function texRoof() {
  const W = 512, H = 512, c = cvs(W, H), x = c.getContext('2d');
  const h = cvs(W, H), hx = h.getContext('2d');
  x.fillStyle = THEME.timber.tileBase; x.fillRect(0, 0, W, H);
  ...
  for (let i = 0; i < ribs; i++) {
    ...
    x.fillStyle = THEME.timber.tileRib; x.fillRect(i * s - 1.2, 0, 1.6, H);
...
function texWisp() {
  const S = 128, c = cvs(S, S), x = c.getContext('2d');
  const g = x.createRadialGradient(S / 2, S / 2, 0, S / 2, S / 2, S / 2);
  THEME.wisp.stops.forEach(s => g.addColorStop(s[0], s[1]));
  x.fillStyle = g; x.fillRect(0, 0, S, S);
  return c;
}
...
function texGrassCutout(seed, opt) {
  ...
  bg.addColorStop(0, THEME.foliage.grassTop); bg.addColorStop(.38, THEME.foliage.grassMid); bg.addColorStop(1, THEME.foliage.grassBase);
  ...
  for (let k = 0; k < blades.length; k++) {
    ...
    const r = (10 + 78 * l + 44 * warm) * (opt.tintR || THEME.foliage.tintR);
  ...
  rb.addColorStop(0, THEME.foliage.bounce); rb.addColorStop(.55, 'rgba(0,0,0,0)'); rb.addColorStop(1, 'rgba(0,0,0,0)');
  x.fillStyle = rb; x.fillRect(0, 0, W, H);
  const sk = x.createLinearGradient(0, H - H * peak * 1.05, 0, H);
  sk.addColorStop(0, THEME.foliage.skyLick); sk.addColorStop(.5, 'rgba(0,0,0,0)'); sk.addColorStop(1, 'rgba(0,0,0,.70)');
...
function texRockCutout(seed, opt) {
  ...
  blobs.forEach((b, bi) => {
    ...
    x.lineWidth = 3 + rnd() * 4; x.strokeStyle = THEME.foliage.rockLit;
    x.beginPath();
    for (let a = 0; a <= 60; a++) {
  ...
  rb.addColorStop(0, THEME.foliage.bounce); rb.addColorStop(.7, 'rgba(0,0,0,0)');
  x.fillStyle = rb; x.fillRect(0, 0, W, H);
  x.globalCompositeOperation = 'source-over';
  return c;
}
...
function texBranchCutout(seed) {
  ...
  function bough(px, py, ang, len, wid, depth) {
    ...
    if (depth < 4 && len > 34) {
      ...
      for (let i = 0; i < THEME.sakura.canopyPerTip; i++)
  ...
  leaves.forEach(l => {
    ...
    const v = l[4];
    tx.fillStyle = mixHex(THEME.sakura.petalLo, THEME.sakura.petalHi, v);
...
function initGL() {
  if (qs('nogl', '0') !== '0') throw new Error('webgl disabled by ?nogl=');
  ...
  renderer.setClearColor(THEME.air.clear, 1);
  ...
  scene = new THREE.Scene();
    /* Density puts the hall back in the air; the colour is what decides how
     ...
     follow it down. */
  scene.fog = new THREE.FogExp2(THEME.air.fog, THEME.air.fogDensity);
  scene.background = new THREE.Color(THEME.air.background);
...
function buildShell() {
  ...
  const wallMat = new THREE.MeshStandardMaterial({
    ...
    roughness: .78, metalness: .05, color: THEME.stone.wallTint
  });
  ...
  const sky = new THREE.Mesh(new THREE.PlaneGeometry(360, 190),
    new THREE.MeshBasicMaterial({ color: hdr(...THEME.sky.tint), map: tx(texSky()),
  ...
  THEME.ridge.layers.forEach((r, i) => {
  ...
  const floorMat = new THREE.MeshStandardMaterial({
    ...
    /* The court used to carry a planar mirror and this was tuned to feed it —
       ...
       reflecting. */
    roughness: THEME.stone.floorRough, metalness: THEME.stone.floorMetal, color: THEME.stone.floorTint
  });
  ...
  const platMat = new THREE.MeshStandardMaterial({
    ...
    normalScale: new THREE.Vector2(.18, .18), roughness: .93, metalness: .02, color: THEME.stone.platTint
  });
...
function buildTemple() {
  ...
  const timber = surface(wallWood(), [4, 1.6], { color: THEME.timber.hallTint, normal: 1.5 });
  const post   = surface(postWood(), [1.1, 1.0], { color: THEME.timber.postTint, normal: 1.05, metalness: .03 });
  const gold   = new THREE.MeshStandardMaterial({ color: THEME.timber.gold, roughness: .38, metalness: .78 });
  ...
  const tileMat = surface(rf, [1, 1], { color: THEME.timber.tileTint, roughness: .74, metalness: .10, normal: 1.4 });
  ...
  const paper = new THREE.MeshBasicMaterial({ color: hdr(...THEME.ember.paperHdr), fog: true, toneMapped: false });
  ...
  const spill = new THREE.Mesh(new THREE.PlaneGeometry(30, 16),
    new THREE.MeshBasicMaterial({ map: tx(texGlow(THEME.ember.hallGlow[0], THEME.ember.hallGlow[1])),
  ...
  const mist = new THREE.Mesh(new THREE.PlaneGeometry(64, 20),
    new THREE.MeshBasicMaterial({ map: tx(texGlow(THEME.weather.hazeGlow[0], THEME.weather.hazeGlow[1])),
...
const MOON = { x: THEME.moon.x, y: THEME.moon.y, z: THEME.moon.z, r: THEME.moon.r };
function buildMoon() {
  const disc = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * 2, MOON.r * 2),
    ...
    new THREE.MeshBasicMaterial({ map: tx(texMoon()), color: hdr(...THEME.moon.hdr),
  ...
  const halo = new THREE.Mesh(new THREE.PlaneGeometry(MOON.r * THEME.moon.haloScale, MOON.r * THEME.moon.haloScale),
...
function buildTorii() {
  ...
  const lac = surface(lib('lacquer', () => texLacquer()), [2, 2],
    { color: hdr(...THEME.torii.hdr), roughness: .92, metalness: .05, normal: .75 });
  const gold = new THREE.MeshStandardMaterial({ color: THEME.torii.gold, roughness: .50, metalness: .60 });
  const g = new THREE.Group();
  ...
  const cap = new THREE.Mesh(sweepPoly(beamPath(5.95, .62, 2.4),
    ...
    new THREE.MeshStandardMaterial({ color: THEME.torii.cap, roughness: .42, metalness: .14 }));
...
function buildLantern(x, z, s, y) {
  const stone = lib('lanternStone', () => surface(lib('granite', () => texStone(17)), [1.5, 1.5],
    { color: THEME.stone.graniteTint, metalness: 0, normal: 1.45 }));
  ...
  const paneMat = new THREE.MeshBasicMaterial({ color: hdr(...THEME.ember.lanternHdr), fog: false, toneMapped: false });
  ...
  const glow = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 3.4),
    new THREE.MeshBasicMaterial({ map: tx(texGlow(THEME.ember.lanternGlow[0], THEME.ember.lanternGlow[1])),
  ...
  const lt = new THREE.PointLight(THEME.light.lantern, THEME.light.lanternI, THEME.light.lanternRange, 2);
...
function buildMaple(seed, x, z, scale) {
  ...
  const trunk = new THREE.Mesh(mergeGeos(parts),
    new THREE.MeshStandardMaterial({ color: THEME.sakura.bark, roughness: .94, metalness: .0, side: THREE.DoubleSide }));
  ...
  const per = LOW ? Math.max(3, THEME.sakura.canopyPerTip - 6) : THEME.sakura.canopyPerTip;
...
function buildRocks() {
  const mat = new THREE.MeshStandardMaterial({ color: THEME.stone.boulder, roughness: .46, metalness: .10 });
...
/* the giant wordmark: removed, page furniture. */

/* --------------------------------------------------------- atmospherics */
function buildAtmosphere() {
  /* drifting haze slabs */
  const hazeTex = tx(texGlow(THEME.weather.hazeGlow[0], THEME.weather.hazeGlow[1]));
  WORLD.haze = [];
  ...
  for (let i = 0; i < (LOW ? 4 : 6); i++) {
    ...
    const h = new THREE.Mesh(new THREE.PlaneGeometry(s, s * .55),
      new THREE.MeshBasicMaterial({ map: hazeTex, transparent: true, blending: THREE.AdditiveBlending,
        depthWrite: false, fog: false, opacity: THEME.weather.hazeOpacity[0] + rnd() * THEME.weather.hazeOpacity[1] }));
  ...
  const emb = new THREE.Points(g, new THREE.ShaderMaterial({
    uniforms: { uT: WORLD.uT, uTex: { value: tx(texGlow(THEME.ember.motes[0], THEME.ember.motes[1])) },
      uSize: { value: vpH() * .5 } },
    ...
    fragmentShader:
      ...
      ' gl_FragColor = vec4(t.rgb*' + glv3(THEME.ember.moteRgb) + ', t.a*vA*0.75); }'
  }));
  emb.frustumCulled = false; emb.renderOrder = 5;
  scene.add(emb); WORLD.embers = emb;

  /* rain in the slot of open sky — off in sakura-moon */
  ...
  WORLD.ripples = [];
  for (let i = 0; i < Math.min(THEME.weather.ripples, LOW ? 6 : 13); i++) {
...
function buildLeafFall() {
  const N = LOW ? Math.round(THEME.sakura.fall.count * .42) : THEME.sakura.fall.count;
  const mat = new THREE.MeshStandardMaterial({
    map: tx(texLeaf()), alphaTest: .42, side: THREE.DoubleSide,
    color: THEME.sakura.petal, roughness: .84, metalness: 0,
    ...
    emissive: THEME.sakura.emissive, emissiveIntensity: THEME.sakura.emissiveIntensity * 2.4
  });
  const inst = new THREE.InstancedMesh(new THREE.PlaneGeometry(THEME.sakura.fall.size, THEME.sakura.fall.size), mat, N);
...
function buildLights() {
  scene.add(new THREE.HemisphereLight(THEME.light.hemiSky, THEME.light.hemiGround, THEME.light.hemi));

  const key = new THREE.DirectionalLight(THEME.light.key, THEME.light.keyI);
  ...
  const moonKey = new THREE.DirectionalLight(THEME.light.moonKey, THEME.light.moonKeyI);
  ...
  const hallL = new THREE.PointLight(THEME.light.hall, THEME.light.hallI, 15, 2);
  hallL.position.set(0, PODIUM + 1.2, TEMPLE_Z + 8.6); scene.add(hallL); WORLD.hallLight = hallL;
  [-1, 1].forEach(s => {
    const w = new THREE.PointLight(THEME.light.wing, THEME.light.wingI, 11, 2);
  ...
  const moonL = new THREE.PointLight(THEME.light.moonPoint, THEME.light.moonPointI, 46, 2);
  moonL.position.set(11.0, 17.0, -24.0); scene.add(moonL); WORLD.moonLight = moonL;

  const fill = new THREE.PointLight(THEME.light.fill, THEME.light.fillI, 30, 2);
  ...
  const stairL = new THREE.PointLight(THEME.light.stair, THEME.light.stairI, 17, 2);
...
function initPost() {
  ...
  POST.bright = new THREE.ShaderMaterial({
    uniforms: { tS: { value: null }, uThr: { value: THEME.post.threshold }, uKnee: { value: THEME.post.knee } },
...
function updateWorld(dt) {
  ...
  if (WORLD.moonHalo) WORLD.moonHalo.material.opacity = THEME.moon.haloOpacity + f * .10 + Math.sin(clock * .34) * .05;
...
function frame(now) {
  ...
  RIG.prog = 0;   /* the hero waypoint; no chapters to scroll through */
...
const JOBS = [
  ...
  ['Hanging the moon', () => { buildMoon(); buildClouds(); }],
